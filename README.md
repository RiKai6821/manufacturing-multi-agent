...
> 用户一句话描述设备异常，系统由 5 个分工 Agent 协作完成
...
| 维度 | 能力 |
|------|------|
| 🤝 多智能体协作 | 协调者-执行者架构，1协调 + 5执行 Agent，自主规划调度 |
...
| 🚀 工程质量 | 24 单元测试 + GitHub Actions CI + 配置中心 + 统一日志 |
...
```
      ...
      (历史经验)   │  规划·并行调度  │   (诊断入库)
        ...
        ┌──────────┬──────┴──────┬──────────┐
   ┌────▼───┐ ┌────▼───┐ ┌───────▼──┐ ┌─────▼────┐ ┌──────────┐
   │数据分析 │ │知识检索 │ │ 质量评审  │ │ 行动执行  │ │保养规划  │  执行层
   │ Agent  │ │ Agent  │ │  Agent   │ │  Agent   │ │ Agent    │ (5个)
   └────┬───┘ └────┬───┘ └────┬─────┘ └─────┬────┘ └────┬─────┘
        │          │          │             │           │
   ┌────▼──────────▼──────────▼─────────────▼───────────▼────┐
